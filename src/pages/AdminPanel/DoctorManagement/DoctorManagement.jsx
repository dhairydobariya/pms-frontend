...
const DoctorManagement = () => {
  ...
  const handleDeleteDoctor = async () => {
    ...
    if (selectedDoctor) {
      try {
        await axios.delete(
          `${import.meta.env.VITE_API_BASE_URLs}/v1/admin/delete-doc-by-admin`,
          {
            headers: {
  ...
  useEffect(() => {
    const delaySearch = setTimeout(() => {
      if (searchTerm) {
        ...
        axios
          .get(
            `${import.meta.env.VITE_API_BASE_URLs}/v1/dashboard-admin/search-doctor-and-patient-list?query=${searchTerm}`,
            {
              headers: {
                Authorization: `Bearer ${token}`, // Include the token in the headers
              },
